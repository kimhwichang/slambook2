...
void ComputeORB(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, vector<DescType> &descriptors) {
  ...
  for (auto &kp: keypoints) {
    ...
    for (int i = 0; i < 8; i++) {
      uint32_t d = 0;
      for (int k = 0; k < 32; k++) {
        int idx_pq = i * 32 + k;   // blob안에 점에 대해 for문 돌면서
        cv::Point2f p(ORB_pattern[idx_pq * 4], ORB_pattern[idx_pq * 4 + 1]);
        cv::Point2f q(ORB_pattern[idx_pq * 4 + 2], ORB_pattern[idx_pq * 4 + 3]); // 임의의 두점 선택
        ...
        cv::Point2f qq = cv::Point2f(cos_theta * q.x - sin_theta * q.y, sin_theta * q.x + cos_theta * q.y)
                         + kp.pt;
        if (img.at<uchar>(pp.y, pp.x) < img.at<uchar>(qq.y, qq.x)) {  // p가 q보다 작으면 1 아니면 0 이런거 128 개 골라서 descriptor 정의
          d |= 1 << k; // or bit 연산자
        }
      }
      desc[i] = d;
    }
    descriptors.push_back(desc);  // keypoint전체에 대한 for문 돌면서 descriptor 하나씩 지정
  }
...
void BfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
  ...
  for (size_t i1 = 0; i1 < desc1.size(); ++i1) {
    ...
    for (size_t i2 = 0; i2 < desc2.size(); ++i2) {
      ...
      int distance = 0;
      for (int k = 0; k < 8; k++) { //8개의 32bit unsigned int data
        distance += _mm_popcnt_u32(desc1[i1][k] ^ desc2[i2][k]);
      }
      if (distance < d_max && distance < m.distance) {
        m.distance = distance;
        m.trainIdx = i2;  // 더 가까운 애가 나타나면 거리를 걔로 하고 매칭 쌍도 걔로 바꾼다.
      }
    }
    if (m.distance < d_max) {  // 최종 threshold 통과하면 최종 선택
      matches.push_back(m);
    }
  }
}