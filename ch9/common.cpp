...
BALProblem::BALProblem(const std::string &filename, bool use_quaternions) {
    ...
    for (int i = 0; i < num_observations_; ++i) {
     ...
     // num_observation 만큼은 camera_idx (pose) , point_idx (land_mark) , observation_x, observation_y (pixel)
     // 그 다음 부터는 9* num_cam 만큼 카메라 parameter / 그 다음 3* point 갯수 만큼 point 위치
